...
void ofApp::keyPressed(int key) {
	...
	if (key == 'd' && isOpen) {
		...
		cout << "Error: FIEE OPEN FIRST" << endl;
	}
...
void ofApp::freeMemory() {
	...
	vector<string>().swap(maze_input);
	//swap 함수를 사용하여 빈 벡터와 maze_input을 교환하여 maze_input을 비운다.
	vector<pair<int, int>>().swap(dfs_path);
	//swap 함수를 사용하여 빈 벡터와 dfs_path를 교환하여 dfs_path를 비운다.
	vector<pair<int, int>>().swap(trial_path);
	//swap 함수를 사용하여 빈 벡터와 trial_path를 교환하여 trial_path를 비운다.
	isdfs = false;
	//isdfs는 DFS가 진행 중인지를 나타낸다. 
	// 여기서는 DFS가 종료되었음을 나타낸다.
